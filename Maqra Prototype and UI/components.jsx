/* ===========================================================================
   Maqra — shared components (exported to window)
   =========================================================================== */
...
function Icon({ name, size = 18, stroke = 1.8, fill = "none", style }) {
  const d = ICONS[name];
  return (
...
function Wordmark({ size = 21 }) {
  return (
    <div className="brand">
      <Mark />
      <span className="name" style={{ fontSize: size }}>Maqra<span>'</span></span>
    </div>
  );
}

/* ---- avatar ---- */
function Avatar({ name, sex = "m", size = 58, ring }) {
  const initials = name.split(" ").filter((w) => !["Bin", "Binti"].includes(w)).slice(0, 2).map((w) => w[0]).join("");
...
function StatusDot({ s, size = 9 }) {
  return <span className="dot" style={{ width: size, height: size, background: `var(--st-${MAQRA.STATUS_MAP[s].css}-ink)` }} />;
}
function StatusChip({ s }) {
  const st = MAQRA.STATUS_MAP[s];
  return (
...
function GredBadge({ g }) {
  const gr = MAQRA.GRED_MAP[g];
  if (!gr) return <span className="badge">{g}</span>;
  return <span className="badge" style={{ color: `var(--gr-${gr.css})`, background: `color-mix(in oklch, var(--gr-${gr.css}) 12%, var(--surface))`, borderColor: "transparent" }}>{g}</span>;
}
  ...
  return (
    <div className="card" style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div>
        ...
        {sub && <div style={{ fontSize: 12.5, color: "var(--ink-3)", marginTop: 5 }}>{sub}</div>}
      </div>
    </div>
  );
}
...
/* ---- sidebar ---- */
function Sidebar({ roleIcon, roleTitle, roleSub, items, active, onNav, onLogout, footerNote }) {
      ...
      <div className="msidebar">
        <div className="mtopbar">
          ...
          <button className="mburger" onClick={() => setOpen((o) => !o)} aria-label="Menu">
            ...
            <span style={{ fontSize: 13, fontWeight: 700 }}>{activeLabel}</span>
          </button>
        </div>
        {open && (
          <>
            ...
            <div className="mdrawer">
              ...
              <nav className="nav">
                {items.map((it) => it.section ? (
                  <div className="lbl" key={"s" + it.section}>{it.section}</div>
                ) : (
                ...
                <button onClick={() => { onLogout(); setOpen(false); }} style={{ marginTop: 6 }}><span className="ic"><Icon name="logout" size={19} /></span>Log Keluar</button>
              </nav>
            </div>
          </>
        )}
  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="nav">
        ...
        {footerNote && <div style={{ fontSize: 11.5, color: "var(--ink-3)", padding: "10px 12px", lineHeight: 1.5 }}>{footerNote}</div>}
        <button onClick={onLogout}><span className="ic"><Icon name="logout" size={19} /></span>Log Keluar</button>
      </nav>
    </aside>
  );
}
...
function FauxQR({ size = 132, seed = 7 }) {
  const cells = useMemo(() => {
    const n = 21, r = (s => () => ((s = (s * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff))(seed * 99 + 1);
    const grid = [];
...
/* ---- progress bar ---- */
function Bar({ value, tone = "var(--accent)", height = 8 }) {
  return (
...
Object.assign(window, {
  Icon, Mark, Wordmark, Avatar, StatusDot, StatusChip, GredBadge,
  StatCard, Sidebar, FauxQR, Bar, Placeholder,
  useState, useEffect, useRef, useMemo, useIsMobile,
});
