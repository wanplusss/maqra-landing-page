/* ===========================================================================
   Maqra — mock data + Quran lookups (exposed on window.MAQRA)
   =========================================================================== */
(function () {
  // ---- status taxonomy (grid cell colors) ----
  const STATUS = [
  ...
  // ---- recitation session categories (history log) ----
  ...
  const GRED_MAP = Object.fromEntries(GRED.map((g) => [g.key, g]));
  ...
  // ---- build a 604-page status map for a student ----
  function buildStatus(frontier, seed, beginner) {
    const map = {};
    const r = rng(seed);
    for (let p = 1; p <= 604; p++) {
      if (p > frontier + 9) { map[p] = "belum"; continue; }
      if (p > frontier)     { map[p] = "bacaan"; continue; }       // active reading band
      // memorized region — mostly hafazan, with revision overlay
      const x = r();
      if (beginner && p > frontier - 6) { map[p] = x < 0.5 ? "iqra" : "tilawah"; continue; }
      if (x < 0.07)      map[p] = "syahadah";   // examined
      else if (x < 0.20) map[p] = "talaqqi";    // verified w/ teacher
      else if (x < 0.40) map[p] = "murajaah";   // under revision
      else               map[p] = "hafazan";
    }
    return map;
  }
  ...
  function tally(map) {
    const t = {};
    STATUS.forEach((s) => (t[s.key] = 0));
    for (let p = 1; p <= 604; p++) t[map[p]]++;
    return t;
  }

  // ---- ulasan / masalah / cadangan pools (Bahasa Melayu) ----
  const ULASAN = [
  ...
  function buildHistory(frontier, seed) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      out.push({
        ...
        guru: ["Ustazah Aisyah", "Ustaz Hakim", "Ustazah Mariam"][Math.floor(r() * 3)],
      });
      page = from;
    }
    return out;
  ...
  const students = RAW.map((s, i) => {
    ...
    return {
      ...
      status,
      tally: t,
      juzuk: juzukOf(s.frontier),
      surah: surahOf(s.frontier),
      lastHafazan: s.frontier - 1,
      lastHafazanSurah: surahOf(s.frontier - 1),
      progress: Math.round((s.frontier / 604) * 1000) / 10,
      history: buildHistory(s.frontier, 1000 + i * 97),
    };
  });
  ...
  const school = {
    ...
    description:
      "Maahad Tahfiz Al-Furqan menyediakan program hafazan Al-Quran 30 juzuk dengan pendekatan talaqqi dan musyafahah. Kami komited membentuk generasi huffaz yang berakhlak mulia dan cemerlang dalam akademik.",
    phone: "03-8765 4321",
    email: "admin@alfurqan.edu.my",
    founded: 2016,
    enrolled: students.length,
    teachers: [
      { id: "t1", name: "Ustazah Aisyah Binti Hamzah", email: "aisyah@alfurqan.edu.my", kelas: "Tahun 4 & 5" },
      { id: "t2", name: "Ustaz Hakim Bin Rashid",      email: "hakim@alfurqan.edu.my",  kelas: "Tahun 2 & 3" },
      { id: "t3", name: "Ustazah Mariam Binti Idris",   email: "mariam@alfurqan.edu.my", kelas: "Tahun 6" },
    ],
    announcements: [
      { id: "a1", date: "2026-05-28", tag: "Peperiksaan", title: "Tasmik Pertengahan Tahun Bermula 9 Jun",
        body: "Sesi tasmik penilaian pertengahan tahun akan berlangsung dari 9 hingga 20 Jun. Pelajar dinasihatkan memperbanyakkan murajaah juzuk lepas." },
      { id: "a2", date: "2026-05-15", tag: "Majlis", title: "Majlis Khatam & Penyampaian Sijil",
        body: "Majlis khatam tahunan akan diadakan pada 5 Julai 2026, jam 9 pagi di dewan utama maahad. Kehadiran ibu bapa amat dialu-alukan." },
      { id: "a3", date: "2026-04-30", tag: "Cuti", title: "Cuti Pertengahan Penggal",
        body: "Maahad akan bercuti dari 24 Mei hingga 1 Jun. Kelas tahfiz disambung semula pada 2 Jun 2026." },
    ],
  };

  // ---- platform-wide schools (super-admin / owner view) ----
  const avgProgress = Math.round(students.reduce((a, s) => a + s.progress, 0) / students.length * 10) / 10;
  const schools = [
    { slug: "al-furqan", name: "Maahad Tahfiz Al-Furqan", city: "Kajang, Selangor", students: students.length, teachers: school.teachers.length, avgProg: avgProgress, plan: "Premium", status: "aktif", since: "2024-01" },
  ...
  window.MAQRA = {
    STATUS, STATUS_MAP, KATEGORI, GRED, GRED_MAP,
    juzukOf, juzPages, surahOf, JUZ_START,
