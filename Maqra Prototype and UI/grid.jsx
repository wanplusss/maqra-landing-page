...
function Legend({ compact }) {
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: compact ? "6px 14px" : "8px 18px", alignItems: "center" }}>
      {MAQRA.STATUS.map((s) => (
        <span key={s.key} style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 12.5, fontWeight: 600, color: "var(--ink-2)" }}>
...
function GridTooltip({ data }) {
  ...
  const st = MAQRA.STATUS_MAP[status];
    ...
    <div style={style} className="animate-up">
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
          ...
          <span className="mono" style={{ fontSize: 11, opacity: 0.65 }}>Juz {MAQRA.juzukOf(page)}</span>
        </div>
        <div style={{ fontSize: 12.5, opacity: 0.8, marginBottom: 9 }}>Surah {MAQRA.surahOf(page)}</div>
...
function PageGrid({ statusMap, columns = 20, onCellClick, selected, juzMarkers = true }) {
  const [hover, setHover] = useState(null);

  const enter = (e, page) => {
    const rect = e.currentTarget.getBoundingClientRect();
    setHover({ rect, page, status: statusMap[page] });
  };
    ...
    <div style={{ position: "relative" }} onMouseLeave={() => setHover(null)}>
      <div className="pagegrid" style={{ display: "grid", gridTemplateColumns: `repeat(${columns}, minmax(0,1fr))`, gap: 5 }}>
        {Array.from({ length: 604 }, (_, i) => {
          ...
          const css = MAQRA.STATUS_MAP[s].css;
          const isJuz = juzMarkers && MAQRA.JUZ_START.includes(page);
          ...
          return (
            <button
              ...
              style={{
                ...
                fontFamily: "var(--font-mono)", fontSize: columns > 24 ? 9.5 : columns > 18 ? 10.5 : 11.5,
              ...
              onMouseOver={(e) => { if (onCellClick) { e.currentTarget.style.transform = "scale(1.18)"; e.currentTarget.style.boxShadow = "0 4px 12px -2px hsl(var(--shadow-color) / 0.3)"; e.currentTarget.style.zIndex = 3; } }}
              onMouseOut={(e) => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = ""; e.currentTarget.style.zIndex = sel ? 2 : ""; }}
            >
              {page}
            </button>
          );
...
/* compact mini-grid for list rows / cards (no numbers) */
function MiniGrid({ statusMap, columns = 38, cell = 5 }) {
  return (
    <div className="minigrid" style={{ display: "grid", gridTemplateColumns: `repeat(${columns}, 1fr)`, gap: 1.5 }}>
      {Array.from({ length: 604 }, (_, i) => {
        const css = MAQRA.STATUS_MAP[statusMap[i + 1] || "belum"].css;
        return <span key={i} style={{ aspectRatio: "1/1", borderRadius: 1.5, background: `var(--st-${css}-ink)`, opacity: statusMap[i + 1] === "belum" ? 0.18 : 0.85 }} />;
      })}
    </div>
  );
}

Object.assign(window, { Legend, PageGrid, MiniGrid });
