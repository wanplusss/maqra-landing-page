...
export function StudentRow({ st, onOpen }) {
  ...
  return (
    <button
      onClick={onOpen}
      style={{
        display: "flex", alignItems: "center", gap: 20, width: "100%", textAlign: "left", flexWrap: "wrap",
        background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 16, padding: "18px 22px",
    ...
    >
      <Avatar name={st.name} sex={st.sex} size={50} />
      <div style={{ width: 230, flex: "none" }}>
        <div style={{ fontWeight: 700, fontSize: "15px", color: "var(--ink)" }}>{st.name}</div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, fontSize: "12.5px" }}>
          <span style={{ color: "var(--ink-2)", fontWeight: 600 }}>Sasaran: {st.target} m.s.</span>
          <span className="mono" style={{ color: "var(--ink-2)", fontWeight: 700 }}>{st.progress}%</span>
        </div>
