...
export function TweaksPanel({ title = "Ubah Suai", children }) {
  const [open, setOpen] = useState(true); // default open for prototype exploration
    ...
    return (
      <button 
        ...
        style={{
          ...
          background: "var(--accent)", color: "#fff", display: "flex", gap: 6,
          alignItems: "center", borderRadius: "50%", width: 44, height: 44, 
          justifyContent: "center", boxShadow: "0 4px 12px rgba(0,0,0,0.15)"
        }}
        title="Buka Panel Ubah Suai"
      >
        ⚙️
      </button>
    );
...
export function TweakRadio({ label, value, options, onChange }) {
  ...
  return (
    <TweakRow label={label}>
      <div ref={trackRef} className="twk-seg" style={{ display: "flex" }}>
        <div className="twk-seg-thumb"
             style={{ left: `calc(2px + ${idx} * (100% - 4px) / ${n})`,
                      width: `calc((100% - 4px) / ${n})` }} />
        {opts.map((o) => (
          <button key={o.value} type="button" onClick={() => onSelect(o.value)} style={{ position: "relative", zIndex: 2 }}>
            {o.label}
          </button>
        ))}
      </div>
...
const __TwkCheck = ({ light }) => (
  <svg viewBox="0 0 14 14" style={{ position: "absolute", top: 6, left: 6, width: 13, height: 13 }} aria-hidden="true">
        ...
        {options.map((o, i) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" data-on={on ? "1" : "0"}
                    style={{ background: o, position: "relative", height: 34, borderRadius: 6 }}
                    onClick={() => onChange(o)}>
